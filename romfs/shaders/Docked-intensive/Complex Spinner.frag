...
vec2 obj0(in vec3 p){
  ...
  return vec2(min(min(c1,c3),c2),0.0);
}
...
vec2 obj2(vec3 p){
  ...
  p.x-=sin(time*2.0);
  return vec2(length(max(abs(p.xy)-vec2(0.5,0.1),0.0))-0.2,2.0);
}
...
void cameraSetup(out vec3 prp, out vec3 scp){
  float time=iTime+timeadd;
  ...
  vec3 vrp=vec3(sin(camSpeed+0.2)*32.0,0.0,cos(camSpeed+0.2)*32.0);
  float camCenterDist=2.1+exp(-20.0*pow(sin(time*0.2)+1.0,20.0))*2.8;
  prp=vec3(sin(camSpeed)*32.0,0.0,cos(camSpeed)*32.0);
  
  // Create camera matrix manually (since we can't use mat3 constructor easily)
  vec3 camX = normalize(prp);
  vec3 camY = vec3(0.0,1.0,0.0);
  vec3 camZ = normalize(vrp-prp);
  
  vec3 camMovP=vec3(sin(time)*camCenterDist,cos(time)*camCenterDist,0.0);
  prp=prp + camMovP.x*camX + camMovP.y*camY + camMovP.z*camZ;
  float vpd=1.5; 
...
void raymarching(out vec4 fragColor, in vec3 prp, in vec3 scp){
  ...
  float f=1.0;
  for(int i=0;i<96;i++){ // Reduced from 128 for performance
  ...
  if (f<maxd){
    ...
    if (s.y==2.0){
      ...
      s=vec2(0.1,0.0);
      maxd=16.0;
      for(int i=0;i<48;i++){ // Reduced from 64 for performance
