...
void main()
{
  //Camera animation - fixed camera movement since we don't have mouse input
  vec3 vuv=vec3(0.0,1.0,0.0);
  vec3 vrp=vec3(iTime*2.0, sin(iTime)*2.0, 0.0); // Simplified movement
  
  // Fixed camera angles for automatic rotation
  //float mx = iTime * 0.5;
  //float my = sin(iTime * 0.3) * 0.5 + 0.5;
  
  float mx = iTime * 0.2; // Slower rotation
  float my = 0.7; // Good angle to see checkerboard
  
  vec3 prp=vrp+vec3(cos(my)*cos(mx),sin(my),cos(my)*sin(mx))*8.0;
  float vpd=1.5;
  vec3 light=prp+vec3(5.0,3.0,5.0);
