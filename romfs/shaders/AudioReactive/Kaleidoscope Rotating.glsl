...
/* Audio Reactive Kaleidoscope by MrDude with Rotation */
...
void main() {
    ...
    // Rotation controls
    float rotationSpeed = 0.5; // Adjust this for overall rotation speed (0.1 = slow, 2.0 = fast)
    float randomRotation = sin(iTime * 0.3) * 2.0; // Random direction changes
    
    // Audio-reactive zoom - bass makes it zoom in/out
    float zoomOut = 1.0 + sin(iTime * 2.0 + iAudioBass * 5.0) * 0.5;
    
    // Audio-reactive rotation speed with random direction
    float audioRotationSpeed = 0.15 + iAudioMid * 0.3;
    float totalRotation = (r * (audioRotationSpeed * rotationSpeed)) + randomRotation;
    ...
    for (int i = 0; i < 50; i++) {
        ...
        // Apply overall rotation to the entire pattern
        zoomedUV *= rotate(totalRotation * 0.5);
